...
module Init =
    ...
    module Db =
        ...
        let runCreate connectString cancelToken ops =
            match ops with
            | [] ->
                task { return Ok () }
            | ops ->
                taskTryEx (Sql.write  connectString cancelToken) ops


        let init connectString cancelToken =
            task {
                let getTables = [NpgsqlBatchCommand(DbInit.getTableNames)]
                match! taskTryEx (Sql.read connectString cancelToken ReadFn.list<string>) getTables with
                | Error ex -> return Error ex
                | Ok tables ->
                    let ops = getSqlOps tables
                    match! runCreate connectString cancelToken ops with
                    | Error ex -> return Error ex
                    | Ok () ->
                        match! taskTryEx (Sql.read connectString cancelToken ReadFn.tryFirst<Pos>) [NpgsqlBatchCommand(DbInit.getPositions)] with
                        | Error ex -> return Error ex
                        | Ok None -> return Error (exn "position counter missing")
                        | Ok (Some pos) ->
                            if pos.Event <> pos.Counter then
                                return! taskTryEx (Sql.write connectString cancelToken) [NpgsqlBatchCommand(DbInit.syncPosition)]
                            else
                                return Ok ()
            }
