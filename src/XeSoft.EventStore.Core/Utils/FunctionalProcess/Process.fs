...
type Process<'initArg, 'model, 'msg, 'effect> = {
...
module Process =
    ...
    let test (proc: Process<'initArg, 'model, 'msg, 'effect>) (model: 'model) (msgs: List<'msg>) =
        let init = model, []
    ...
    let run (sideEffects: SideEffects<'effect, 'msg>) (proc: Process<'initArg, 'model, 'msg, 'effect>) (initArg: 'initArg) : Task<Result<'model, exn>> =
        task {
            try
                let initModel, initMsg = proc.Init initArg
                // while b/c recursive Task doesn't optimize to while loop, and can result in OOM exception
